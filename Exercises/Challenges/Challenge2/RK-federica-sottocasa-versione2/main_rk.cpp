...
if(strcmp(args[1],"RK45")==0) { c2=1./64.; //parameter for increasing time step
					  // I need to have a sufficient decrease of the local  error to allow time step coarsening  
                               maxSteps=10000;
      result= rk(fun,args[1],t0,T,y0,h_init,(T-t0)/4.,errorDesired, status,c2,rk45_step,maxSteps);   
			          }
...
if(strcmp(args[1],"RK23")==0) {
				...
				maxSteps=1000000;
      result= rk(fun,args[1],t0,T,y0,h_init,(T-t0)/4.,errorDesired, status,c2,rk23_step,maxSteps);   
