...
public:
  ...
  size (void)
  {
    ...
    return (ret+1); //I've corrected it:using isLast size forgets about last node
  }
  ...
  {  //NOTE: I added the if conditions otherwise I get errors replacing the first and the last node

   if(previous!=NULL) previous->setNext (replacement); 
   replacement->setPrevious(previous);
   if(next!=NULL) next->setPrevious (replacement);
